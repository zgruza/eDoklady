...
let logoutTimeout;
function autoLogout(){logoutTimeout=setTimeout(function(){window.location.href='index.html?autologout';},60000);}
function stopAutoLogout(){clearTimeout(logoutTimeout);}
document.addEventListener('swup:contentReplaced',function(){
    if(!document.location.pathname.includes('index')){autoLogout();}else{stopAutoLogout();}
    if(document.location.pathname.includes('profile_p')){document.getElementById('apply_default').click();document.getElementById('profile_name').innerText = localStorage.getItem('given_name') + " "+localStorage.getItem('family_name');
        const FINGERPRINT=localStorage.getItem('FINGERPRINT');
        if(FINGERPRINT&&FINGERPRINT=="yes"){document.getElementById('switch-bio').checked=true}
    }
});
if(!document.location.pathname.includes('index')){autoLogout();}else{stopAutoLogout();}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var pwaName = "eDoklady"; //Local Storage Names for PWA
    ...
    function init_template(){
        ...
        if(!addOfflineClasses.length){
            ...
            offlineAlert.textContent = 'Nebylo zjištěno připojení k internetu';
            onlineAlert.className = 'online-message bg-green-dark color-white';
            onlineAlert.textContent = 'Jste zpět online';
